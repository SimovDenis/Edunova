...
public class ObradaPolaznik {
	...
	private void ucitajStavkuIzbornika() {
		switch (Pomocno.unosRasponBroja("Odaberi stavku polaznik izbornika: ", "Odabir mora biti 1-5", 1, 5)) {
		case 1:
			pregledPolaznika();
			prikaziIzbornik();
			break;
	...
	private void dodavanjePolaznika() {
		...
		p.setSifra(Pomocno.unosSifrePolaznika(polaznici));
	...
	private void promjenaPolaznika() {
		boolean upit = Pomocno.unosBoolean(
				"Jeste li sigurni da želite promijeniti jednog od polaznika? (da za nastavak ili bilo što za odustajanje): ");
		if (upit) {
			if (polaznici.isEmpty()) {
				System.out.println("\nTrenutno nije unesen niti jedan polaznik");
			} else {
				pregledPolaznika();
				...
				p.setBrojUgovora(Pomocno.unosString("Unesi Broj ugovora polaznika (" + p.getBrojUgovora() + "): ",
						"Broj ugovora obavezno obavezno"));
			}
		}
	...
	private void brisanjePolaznika() {
		boolean upit = Pomocno.unosBoolean(
				"Jeste li sigurni da želite obrisati jednog od polaznika? (da za nastavak ili bilo što za odustajanje): ");
		if (upit) {
			if (polaznici.isEmpty()) {
				System.out.println("\nTrenutno nije unesen niti jedan polaznik");
			} else {
